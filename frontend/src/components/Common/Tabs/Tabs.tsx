...
export const Tabs: FC<TabsProps> = ({ children, forcedActiveTab, classNames, fixed }) => {
  ...
  const getButtonClasses = (label: string) =>
    clsx(
      "w-full flex flex-1 items-center justify-center p-2 text-gray-700 font-medium rounded-t-[10px] shadow-[rgba(50,50,93,0.25)_0px_6px_12px_-2px,_rgba(0,0,0,0.3)_0px_3px_7px_-3px] min-w-[75px] bg-neutral dark:bg-base-100 relative",
      {
  ...
  return (
    <div className={`mx-auto w-full ${classNames}`}>
      <div className="w-full bg-transparent pb-csm">
        <div
          className={`bg-transparent flex max-h-[40px] w-[96%] border-gray-300 lg:max-h-[50px] 2xl:w-[97%] ${
